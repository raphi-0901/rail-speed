...
export class ComfortJetProvider implements SpeedProvider {
    readonly name = 'ComfortJet';
    ...
    fetch(): { promise: Promise<ProviderResult>; cancel: () => void } {
        const {promise, cancel} = this._http.fetchText('http://cdwifi.cz/portal/api/vehicle/realtime', {
        ...
        const wrappedPromise = (async (): Promise<ProviderResult> => {
            try {
                ...
                const speed = JSON.parse(text)?.speed;

                if (!Number.isFinite(speed)) {
                    throw new Error(`${this.name}: invalid numeric response`);
                }
